...
@section('title','Closed Enrollments')
@section('page-title','Closed Enrollments')
...
.avatar-sm{width:34px;height:34px;border-radius:50%;object-fit:cover;background:#64748b;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff;flex-shrink:0;overflow:hidden}
...
<div class="gt-card">
  <div class="gt-card-header" style="flex-wrap:wrap;gap:10px;">
    <div class="gt-card-title">Closed Enrollments</div>
    <div style="display:flex;align-items:center;gap:8px;margin-left:auto;">
      <form id="filter-form" method="GET" action="{{ route('institute.students.closed') }}" style="display:contents;">
      ...
      <tbody>
        @forelse($enrollments as $enr)
          ...
          <tr>
            ...
            <td style="font-size:12px;color:var(--text-2);">{{ $enr->enrollment_no ?? '—' }}</td>
            <td>
              @if($stu)
        ...
        @empty
          <tr><td colspan="6" style="text-align:center;padding:40px;color:var(--text-2);">No closed enrollments.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
