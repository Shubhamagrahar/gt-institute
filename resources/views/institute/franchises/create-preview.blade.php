...
@section('topbar-actions')
  <a href="{{ route('institute.franchises.charges') }}" class="btn btn-outline btn-sm">← Edit Course Access</a>
@endsection
...
<div class="frn-preview-layout">
  ...
  {{-- ── LEFT: Details + Course Access + Fee ────────────────────── --}}
  <div class="frn-preview-left">
    ...
    <div class="gt-card frn-detail-card">
      ...
      <div class="frn-detail-grid">
        ...
        <div class="frn-detail-item">
          ...
          <span class="frn-detail-value mono">₹{{ number_format($data['low_wallet_alert'] ?? 0, 0) }}</span>
        </div>
    ...
    <div class="gt-card frn-payment-card" style="margin-top:16px;">
      ...
      @else
        <table style="width:100%;border-collapse:collapse;font-size:12.5px;">
          <thead>
            <tr>
              ...
              <th style="text-align:right;color:var(--text-3);font-size:11px;text-transform:uppercase;letter-spacing:.4px;padding:0 0 8px;border-bottom:1px solid var(--border-1);">Adm. Charge</th>
              <th style="text-align:right;color:var(--text-3);font-size:11px;text-transform:uppercase;letter-spacing:.4px;padding:0 0 8px;border-bottom:1px solid var(--border-1);">Cert. Charge</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr>
              <td style="padding:8px 0;border-bottom:1px solid var(--border-1);color:var(--text-1);font-weight:600;font-size:13px;">{{ $ct->name }}</td>
              <td style="padding:8px 0;border-bottom:1px solid var(--border-1);text-align:center;color:var(--text-3);">{{ $ch->course_count ?? 0 }}</td>
        ...
        <div style="font-size:11.5px;color:var(--text-3);margin-top:10px;">
          Charges are deducted from franchise wallet per admission/certificate. Fine-tune per course after creation.
        </div>
      @endif
    </div>
    ...
    <div class="gt-card frn-payment-card" style="margin-top:12px;">
      ...
      <div class="frn-pay-note frn-pay-note-blue">
        Credited to franchise wallet immediately on creation. Deducted per admission &amp; certificate.
      </div>
    </div>
  ...
  <div class="frn-preview-right">
    <div class="gt-card frn-confirm-card">
      ...
      <div class="frn-confirm-checklist">
        ...
        @endif
        @if(!empty($data['_course_type_access']))
          <div class="frn-check-item">✓ {{ count($data['_course_type_access']) }} course type(s) granted</div>
        @endif
        @if($levelFee > 0)
      ...
      <a href="{{ route('institute.franchises.charges') }}" class="frn-back-link">
        ← Go back and edit course access
      </a>
    </div>
  </div>
...
.frn-preview-layout { display:grid;grid-template-columns:1fr 300px;gap:20px;align-items:flex-start; }

.frn-detail-card { padding:0;overflow:hidden; }
.frn-detail-header { display:flex;align-items:flex-start;justify-content:space-between;padding:20px 22px 16px;border-bottom:1px solid var(--border-1);gap:12px; }
...
.frn-mode-badge { font-size:11px;font-weight:600;padding:4px 10px;border-radius:20px;white-space:nowrap;flex-shrink:0;background:rgba(30,120,200,.12);color:#1e78c8;border:1px solid rgba(30,120,200,.25); }
.frn-detail-grid { display:grid;grid-template-columns:1fr 1fr;padding:16px 22px;gap:0; }
.frn-detail-item { padding:10px 0;border-bottom:1px solid var(--border-1,rgba(255,255,255,.05));display:flex;flex-direction:column;gap:3px; }
...
.frn-pay-row { display:flex;justify-content:space-between;align-items:center;padding:10px 0;font-size:13px;border-bottom:1px solid var(--border-1,rgba(255,255,255,.05)); }
.frn-pay-total { border-bottom:none;padding:14px 0 4px; }
.frn-pay-note { font-size:12px;line-height:1.55;padding:10px 14px;border-radius:var(--radius-sm);margin-top:14px; }
...
.frn-confirm-amount-box { background:var(--bg-3);border:1px solid var(--border-2);border-radius:var(--radius);padding:14px 16px;margin-bottom:16px;text-align:center; }
...
.frn-back-link { display:block;text-align:center;margin-top:12px;font-size:12px;color:var(--text-3);text-decoration:none; }
