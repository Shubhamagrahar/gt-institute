...
.qp-result-arrow {
  color: var(--text-3); flex-shrink: 0; margin-left: 6px;
...
.qp-field-label {
  ...
  letter-spacing: .7px; color: var(--text-3); margin-bottom: 4px;
}
.qp-field-val {
...
.qp-proceed-btn:hover {
  opacity: 1; color: #fff;
  box-shadow: 0 6px 20px rgba(108,93,211,.45);
...
<div class="qp-wrap">
  ...
  <div class="qp-search-card">
    ...
    <div class="qp-search-box" id="qp-search-box">
      ...
      <button class="qp-clear-btn" id="qp-clear" title="Clear">
  ...
  <div class="qp-selected" id="qp-selected">
    ...
    <div class="qp-actions">
      ...
      <button class="qp-deselect-btn" id="qp-deselect">
...
(function () {
  ...
  function highlight(text, query) {
    ...
    return escaped.replace(re, '<mark style="background:rgba(108,93,211,.18);color:var(--accent);border-radius:2px;padding:0 1px;">$1</mark>');
  }
  ...
  input.addEventListener('input', function () {
    ...
    showLoading();
    debounceTimer = setTimeout(() => doSearch(val.trim()), DEBOUNCE_MS);
  });

  input.addEventListener('focus', function () {
    searchBox.classList.add('focused');
    if (currentResults.length && this.value.trim().length >= 2) {
      showResultsEl();
    }
  });
  ...
  input.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      selectStudent(currentResults[activeIndex]);
  ...
  function renderError() {
    resultsEl.innerHTML = '<div class="qp-state"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12" y2="16"/></svg><p>Could not fetch results. Please try again.</p></div>';
    showResultsEl();
  }

  function renderResults(data, q) {
    if (!data.length) {
      resultsEl.innerHTML = '<div class="qp-state"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><p>No students found for <strong>"' + escHtml(q) + '"</strong></p></div>';
      showResultsEl();
      return;
    }
    ...
    data.forEach((s, i) => {
      ...
      const enrBadge = s.enrollment_no
        ? `<span class="qp-result-enr">${escHtml(s.enrollment_no)}</span>`
        : '';
      const mobileMeta = s.mobile
        ? `<span><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 2h3a2 2 0 0 1 2 1.72c.13.96.33 1.9.61 2.82a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.92.28 1.86.48 2.82.61A2 2 0 0 1 22 16.92z"/></svg>${highlight(s.mobile, q)}</span>` : '';
      const emailMeta = s.email
        ? `<span><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>${highlight(s.email, q)}</span>` : '';
      ...
      html += `<div class="qp-result-item" data-index="${i}">
        ...
        <div class="qp-result-body">
          ...
          <div class="qp-result-meta">
            <span><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>${highlight(s.user_id, q)}</span>
            ${mobileMeta}
            ${emailMeta}
          </div>
        </div>
    ...
    resultsEl.querySelectorAll('.qp-result-item').forEach((el, i) => {
      el.addEventListener('mousedown', function (e) {
        e.preventDefault();
        selectStudent(currentResults[i]);
      });
    });
  }
  ...
  function selectStudent(s) {
    ...
    const nameEl    = document.getElementById('qp-sel-name');
    const uidEl     = document.getElementById('qp-sel-uid');
    const avaEl     = document.getElementById('qp-sel-ava');
    const mobileEl  = document.getElementById('qp-sel-mobile');
    const emailEl   = document.getElementById('qp-sel-email');
    const useridEl  = document.getElementById('qp-sel-userid');
    const enrEl     = document.getElementById('qp-sel-enr');
    const enrNoEl   = document.getElementById('qp-sel-enrno');

    avaEl.textContent    = initials(s.name);
    nameEl.textContent   = s.name;
    uidEl.textContent    = s.user_id;
    mobileEl.textContent = s.mobile || '—';
    emailEl.textContent  = s.email  || '—';
    useridEl.textContent = s.user_id;
    enrNoEl.textContent  = s.enrollment_no || '—';

    if (s.enrollment_no) {
      enrEl.textContent = s.enrollment_no;
      enrEl.style.display = '';
    } else {
      enrEl.style.display = 'none';
    }
