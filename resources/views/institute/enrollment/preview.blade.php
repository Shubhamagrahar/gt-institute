...
@section('page-title','Step 4 - Review & Save')

@section('content')
<div class="gt-card">
  <div class="gt-card-header">
    <div class="gt-card-title">Seat Booking Preview</div>
  ...
  <div class="gt-grid-2" style="gap:20px;">
    <div>
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-top:12px;">
        @if($profile?->photo)
          <img src="{{ asset($profile->photo) }}" style="width:80px;height:80px;object-fit:cover;border-radius:50%;margin-bottom:8px;">
        @endif
    ...
    <div>
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-top:12px;">
        ...
        <div style="display:flex;gap:8px;">
          ...
          <span class="mono">{{ $courseBook->enrollment_no ?: 'Will generate after required payment' }}</span>
        </div>
        @if($courseBook->batch)
          <div style="display:flex;gap:8px;">
  ...
  <div style="display:flex;gap:10px;">
    <form method="POST" action="{{ route('institute.enrollment.confirm', $courseBook) }}">
      @csrf
      <button type="submit" class="btn btn-primary btn-lg">Save Pending Admission</button>
    </form>
  </div>
</div>
@endsection
