...
(() => {
  ...
  function renderDropdown(q=''){
    ...
    crsDrop.innerHTML=f.length?f.map(c=>`<div class="crs-opt" data-id="${c.id}" data-name="${c.name}" style="padding:9px 14px;font-size:13px;cursor:pointer;border-bottom:1px solid var(--border);">${c.name} <span style="color:var(--text-3);font-size:11px;">(${c.duration}m)</span></div>`).join(''):'<div style="padding:10px 14px;font-size:12px;color:var(--text-2);">No courses found</div>';
