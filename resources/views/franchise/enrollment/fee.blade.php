...
<div style="margin-bottom:14px">
  <a href="{{ route('franchise.enrollment.profile', $courseBook) }}" class="btn btn-outline btn-sm">← Edit Profile</a>
  <a href="{{ route('franchise.enrollment.pending') }}" class="btn btn-outline btn-sm" style="margin-left:6px">All Admissions</a>
</div>

<form method="POST" action="{{ route('franchise.enrollment.save-fee', $courseBook) }}" id="fee-form">
  @csrf
      ...
      <div class="pay-body">
        @if($plans->isEmpty())
          <div class="gt-alert gt-alert-warning" style="margin-bottom:16px;">
            Payment plans are not configured. Please contact institute admin to create active payment plans.
          </div>
        @endif
...
function recalc() {
  ...
  document.querySelectorAll('.fee-row').forEach(row => {
    ...
    totalDiscount+=discAmt;grandTotal+=final;
  ...
  }else{monthlyLine.style.display='none';}
...
function updatePaymentSuggestion(grandTotal,requiredFee) {
  ...
  let suggestedAmount=grandTotal,hint='';
  if(activeType==='OTP'){
    suggestedAmount=grandTotal;hint='Full amount — One Time Payment';
    payAmountEl.readOnly=true;payAmountEl.value=suggestedAmount.toFixed(2);
  }else if(activeType==='PART'){
    hint='Koi bhi advance amount enter karo. Total fee: '+money(grandTotal)+'.';
...
document.querySelectorAll('.type-card').forEach(card=>{
  card.addEventListener('click',()=>{
    ...
    help.textContent=card.dataset.type==='OTP'
      ?'The full amount will be paid in a single payment.'
      :card.dataset.type==='PART'
        ?'A partial amount will be collected and remaining due will be tracked.'
...
payAmountEl?.addEventListener('input',()=>{
  ...
  const s=document.getElementById('summary-pay-now');
  if(s) s.textContent=money(parseFloat(payAmountEl.value)||0);
});
